...
##########################################################
##########################################################
### author javid based on Corey and Stephanie's scripts###
# This version has Steph's edits
#
# data set needs to be in bids format: every subject has folders named anat/ and func/
# You need to change the variables at the beginning of the file (e.g., slice_time, nslices, matlab, ..)
# step 1 is skull stripping and will generate optiBET files. 
# step 2 includes linear registration to common step and we keep the file that has _3rdpass.grd in rest of analysis. In this part we ignore files contains _mask or _weight
# step 3 is slice time correction and if you specify this variable with none we will ignore this step. 
# step 4 is motion correction and Dustin changed moco_session3.csh and we don't need extra mean functional step.
# Step 5 and 6 (linear register to skull stripped file and building connectivity) as you see being done by generating the corresponding setup files and xmlg files. 
#
# you can check appropriate connectivity matrices in func/ folders
#
##########################################################
##########################################################
##########################################################
...
nslices = 32; 
tr = 2; 
slice_time = 'ascending' # [ascending descending descending_interleaved none] 
use_old_flags= 0
dataset_name = "tom"
skip_frames_on = 0
	...
	if(any(char.isdigit() and 'anat' in folder[0] for char in folder[0])): # anatomical
		...
		for file in os.listdir(anat_dir):# step 1: skull stripping
			if file.endswith(".gz") and "optiBET" not in file:
                                print('Step 1: skull stripping for subject %s'%sub_ID)
		...
		for file in os.listdir(anat_dir): # step 2: non-linear register to common space
			if file.endswith("optiBET_brain.nii.gz"):
                                print('Step 2: non-linear registeration for subject %s'%sub_ID)
		...
		for file in os.listdir(func_dir): # step 3: slice time correction
			if file.endswith(".nii.gz") and slice_time !='none':
                            	print('step3: slice time correction for file %s'%file)
	        ...
	        for file in os.listdir(func_dir): # step 4: motion correction
		       if file.endswith(".nii") and file.startswith('A') and 'R_' not in file:
			        print("step4: motion correction for file %s"%file)
                ...
                for file in os.listdir(func_dir): # step 5-0: find mean functional image
			if file.endswith(".hdr"):
				subprocess.call(["pxtonifti.tcl",os.path.join(func_dir,file)])
			if file.endswith(".nii") and file.startswith("mean"):
                            mean_moco_file = file
                for file in os.listdir(anat_dir): # step 5: linear registeration to skull-stripped anatomical image
		...
		print('preprocessing for subject %s is done.'%sub_ID)
